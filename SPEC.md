...
- **Obfuscation:** The envelope is not stored as plain JSON — otherwise pausing a game and reading the mine positions out of the dev tools would be a trivial cheat. Before storage every character is xor'd with a keystream (MINSTD LCG seeded from the combined salts) and written as two hex digits; on load the blob is deobfuscated before decoding. Like the checksum this is deliberately obfuscation, not cryptography: salt and algorithm live on the client. Plain JSON saves written before this scheme (detected by a leading `{`) are still accepted for migration.
